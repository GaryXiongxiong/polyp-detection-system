...
class Dataset(object):
    ...
    def __init__(self, data_path, batch_size):
        ...
        self.data_pattern = re.compile(r'^\S+(\s([0-9.]+,){4,5}[0-9.]+)*$')
        self.annot_path = data_path
        self.input_sizes = cfg.YOLO.INPUT_SIZE
        self.batch_size = batch_size
        self.iou_threshold = cfg.YOLO.DATA_IOU_THRESH

        self.train_input_size = cfg.YOLO.INPUT_SIZE
        self.strides = np.array(cfg.YOLO.STRIDES)
        self.classes = util.read_class_names(cfg.YOLO.CLASSES)
        self.num_classes = len(self.classes)
        self.anchors = np.array(util.get_anchors(cfg.YOLO.ANCHORS))
    ...
    def load_annotations(self):
        with open(self.annot_path, 'r') as f:
            ...
            annotations = [line.strip()
                           for line in txt if self.data_pattern.match(line.strip())]
    ...
    def parse_annotation(self, annotation):
        ...
        image_path = os.path.join(os.path.dirname(self.annot_path), line[0])
        if not os.path.exists(image_path):
            raise KeyError("%s does not exist ... " % image_path)
        image = io.imread(image_path)
        bboxes = np.array([list(map(int, box.split(',')[:5])) for box in line[1:]])

        image, bboxes = util.image_preporcess(
